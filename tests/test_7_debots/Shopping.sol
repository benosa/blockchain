...
contract Shopping is BaseDebot {
    ...
    function _menu() internal override {
        ...
        Menu.select(
            ...
            [
                ...
                MenuItem("Buy item","",tvm.functionId(byItem))
            ]
        );
    }

    function byItem(uint32 index) public {
        index = index;
        Terminal.input(tvm.functionId(setTitle), "Tutle in one line please:", false);
        Terminal.input(tvm.functionId(setPrice), "Price in one line please:", false);
        byItem_(title, price);
    }
    ...
    function byItem_(string title, uint quantity) public view {
